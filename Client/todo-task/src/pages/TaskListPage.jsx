...
function TaskListPage() {
  ...
  const addTask = async () => {
    if (newTask.trim()) {
      try {
        ...
        // First update the local state with the new task
        setTasks((prevTasks) => [
          ...prevTasks,
          { ...response.data, title: newTask, completed: false },
        ]);
        // Then fetch the latest tasks from the server
        const fetchTasks = async () => {
          try {
            const response = await axios.get("http://localhost:5000/api/tasks/", {
              headers: {
  ...
  const updateTask = async (id) => {
    try {
      ...
      setTasks(prevTasks => 
        prevTasks.map(task => 
          task._id === id ? { ...task, title: editingText } : task
        )
      );
  ...
  const toggleComplete = async (id, completed) => {
    try {
      ...
      // Find the task and update its completed state
      const taskIndex = updatedTasks.findIndex(task => task._id === id);
      if (taskIndex !== -1) {
        updatedTasks[taskIndex] = { ...updatedTasks[taskIndex], completed: !completed };
              ...
              <div className="flex items-center">
                ...
                >
                  <input
                    ...
                    onChange={() => {}} // Empty onChange to prevent native behavior
